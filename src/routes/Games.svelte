<script lang="ts">
	...
	function findMarket(event: OddsEvent, key: string) {
		for (const bk of event.bookmakers) {
			...
			if (m) return { book: bk.title, market: m };
		}
		return null;
	}

	function spreadFor(event: OddsEvent, team: string): Outcome | null {
		const pick = findMarket(event, 'spreads');
		return pick?.market.outcomes.find((o) => o.name === team) ?? null;
	}

	function mlFor(event: OddsEvent, team: string): Outcome | null {
		const pick = findMarket(event, 'h2h');
		return pick?.market.outcomes.find((o) => o.name === team) ?? null;
	}

	function totalLine(event: OddsEvent): number | null {
		const pick = findMarket(event, 'totals');
		return pick?.market.outcomes[0]?.point ?? null;
	}
</script>
...
{#if loading && !games}
	<div
		class="rounded-2xl border border-neutral-800 bg-neutral-900/60 p-6 text-center text-neutral-400"
	>
		Loading games...
	</div>
{:else if error}
		...
		{#each games as game (game.id)}
			<article class="rounded-2xl border border-neutral-800 bg-neutral-900/60 p-4">
				...
				<div class="space-y-2">
					{#each [game.away_team, game.home_team] as team}
						...
						<div class="flex items-center justify-between">
							<div class="flex items-center gap-3">
								<span class="inline-block w-10 font-mono text-xs text-neutral-500"
									>{teamAbbr(team)}</span
								>
					...
					{#if total !== null}
						<div
							class="mt-2 flex items-center justify-between border-t border-neutral-800 pt-2 text-xs"
						>
