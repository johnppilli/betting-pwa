		...
		<div class="flex gap-2">
			{#each games as g (g.id)}
				<button
					...
					class="shrink-0 rounded-full border px-3 py-1.5 text-xs whitespace-nowrap transition {selectedId ===
					g.id
...
{:else if propsEvent && players.length === 0}
	...
	>
		...
		<p class="mt-1 text-xs text-neutral-600">
			Sportsbooks usually post props closer to tip-off ({propsEvent &&
				tipoffTime(propsEvent.commence_time)}).
		</p>
	</div>
{:else if propsEvent}
